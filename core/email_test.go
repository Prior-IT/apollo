...
func FuzzEmail(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, value string) {
		email, err := core.NewEmailAddress(value)
		// We're not looking for valid email addresses here but rather for unexpected errors leading to a panic
		if err != nil {
			assert.Nil(t, email, "If there is an error, there should not be an e-mail address")
		}
		if email != nil {
...
func TestEmail(t *testing.T) {
	t.Run("ok: email should be case-insensitive", func(t *testing.T) {
		...
		email1, err := core.NewEmailAddress(value)
		assert.Nil(t, err)

		email2, err := core.NewEmailAddress(strings.ToUpper(value))
		assert.Nil(t, err)
	...
	t.Run("err: invalid email addresses", func(t *testing.T) {
		for _, value := range []string{
			...
			_, err := core.NewEmailAddress(value)
